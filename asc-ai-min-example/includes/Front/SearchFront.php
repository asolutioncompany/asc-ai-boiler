...
use ASC\AI_MIN_EXAMPLE\Core\ArchiveConfig;
...
class SearchFront {
	...
	public function redirect_disabled_pages(): void {
		$search_enabled = (bool) \ASC_AI_MIN_EXAMPLE_SEARCH_ENABLED;
		$archive_enabled = (bool) \ASC_AI_MIN_EXAMPLE_ARCHIVE_ENABLED;
	...
	public function filter_main_markup( mixed $markup ): ?string {
		...
		if ( is_search() && (bool) \ASC_AI_MIN_EXAMPLE_SEARCH_ENABLED ) {
			return $this->render_search();
		}

		if ( is_archive() && (bool) \ASC_AI_MIN_EXAMPLE_ARCHIVE_ENABLED ) {
	...
	private function render_search(): string {
		$heading_text = sprintf(
			...
			esc_html__( 'Search results for: %s', \ASC_AI_MIN_EXAMPLE_TEXT_DOMAIN ),
		...
		if ( ! have_posts() ) {
			return '<section class="example-full-content">'
				...
				. '<p class="example-archive-empty">' . esc_html__( 'No results found.', \ASC_AI_MIN_EXAMPLE_TEXT_DOMAIN ) . '</p>'
				. '</div>'
				. '</div>'
				. '</section>';
		}
	...
	private function render_archive(): string {
		...
		if ( ! have_posts() ) {
			return '<section class="example-full-content">'
				...
				. '<div class="example-section example-card-section example-archive-listing-card-section">'
				. '<div class="example-card-section-content">'
				. '<p class="example-archive-empty">' . esc_html__( 'No posts found.', \ASC_AI_MIN_EXAMPLE_TEXT_DOMAIN ) . '</p>'
				. '</div>'
				. '</div>'
				. '</section>';
		}
