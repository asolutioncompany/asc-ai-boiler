...
class CoreSettings {
	 ...
	 * @var string
	 */
	public const OPTION_KEY = 'min_example_site_settings';
	 ...
	 * @var string
	 */
	public const ADMIN_SETTINGS_PAGE_SLUG = 'min-example-settings';
	...
	public static function get_image_url( string $key ): string {
		...
		// Fallback to static manifest path
		$bindings = Media::get_registered_bindings();
		foreach ( $bindings as $binding ) {
			$match = isset( $binding['target'], $binding['key'] )
				&& 'setting' === $binding['target']
				&& $key === $binding['key'];

			if ( $match ) {
				$filename = (string) $binding['media_filename'];
				$url = Media::get_attachment_url_for_path( $filename );
				if ( '' !== $url ) {
					return $url;
				}
			}
