...
class SettingsPage {
	...
	public function handle_save(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to update settings.', \ASC_AI_EXAMPLE_TEXT_DOMAIN ) );
		}
		...
		<div class="wrap example-settings-page example-admin-settings-hub">
			...
			<?php if ( isset( $_GET['settings-updated'] ) ) : ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'Settings saved.', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?></p>
				</div>
			<?php endif; ?>

			<h2><?php esc_html_e( 'Content', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?></h2>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="example-settings-page__default-form">
				...
				<h2><?php esc_html_e( 'Default Images', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?></h2>
				<p>
					<?php esc_html_e( 'Images should have a 4:3 aspect ratio. Recommended image size: 1440x1080. Setting the alt text description is advised for selected images.', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?>
				</p>
						...
						<?php foreach ( $image_fields as $field ) : ?>
							...
							<tr>
								...
								<td>
									...
									>
										<?php esc_html_e( 'Select Image', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?>
									</button>
									<button
										type="button"
										...
										data-target-input="<?php echo esc_attr( $key ); ?>"
									>
										<?php esc_html_e( 'Clear', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?>
									</button>
									<p class="description">
										<?php esc_html_e( 'Attachment ID:', \ASC_AI_EXAMPLE_TEXT_DOMAIN ); ?>
				...
				<?php submit_button( __( 'Save default images', \ASC_AI_EXAMPLE_TEXT_DOMAIN ) ); ?>
			</form>
		</div>
		<?php
	}
	...
	private function get_content_admin_links(): array {
		...
		$out[] = array(
			...
			'label' => __( 'Pages', \ASC_AI_EXAMPLE_TEXT_DOMAIN ),
		);

		$out[] = array(
			'url' => admin_url( 'edit.php' ),
			'label' => __( 'Posts', \ASC_AI_EXAMPLE_TEXT_DOMAIN ),
		);
	...
	public function handle_ajax_set_default_image(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'You do not have permission to update settings.', \ASC_AI_EXAMPLE_TEXT_DOMAIN ),
				),
				403
			);
		}
		...
		if ( ! in_array( $setting_key, $allowed_keys, true ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Invalid settings key.', \ASC_AI_EXAMPLE_TEXT_DOMAIN ),
				),
				400
			);
		}
