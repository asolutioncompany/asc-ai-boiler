...
class BlogFront {
	...
	public function shortcode_home_blogs(): string {
		...
		echo $this->render_view_all_row(
			...
			esc_html__( 'View All Blogs', \ASC_AI_MIN_EXAMPLE_TEXT_DOMAIN )
		);
		wp_reset_postdata();
	...
	public function shortcode_all_blogs(): string {
		...
		if ( ! $query->have_posts() ) {
			wp_reset_postdata();
			echo '<p class="example-archive-empty">' . esc_html__( 'No posts found.', \ASC_AI_MIN_EXAMPLE_TEXT_DOMAIN ) . '</p>';
		} else {
	...
	private function render_post_teaser( int $post_id ): string {
		...
		$excerpt_markup = '';
		$excerpt_source = \ASC_AI_MIN_EXAMPLE_CARD_EXCERPT_SOURCE;
		if ( 'excerpt' === $excerpt_source ) {
			...
			$word_limit = \ASC_AI_MIN_EXAMPLE_CARD_WORD_LIMIT;
			$char_limit = \ASC_AI_MIN_EXAMPLE_CARD_CHAR_LIMIT;
			$excerpt_text = '';
			if ( $word_limit > 0 ) {
