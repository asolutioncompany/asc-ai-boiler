...
class SettingsPage {
	 ...
	 * @var string
	 */
	private const SAVE_ACTION = 'example_site_save_settings';

	private const AJAX_SET_IMAGE_ACTION = 'example_site_set_default_image';
	 ...
	 * @var string
	 */
	private const NONCE_ACTION = 'example_site_settings_save';
	...
	public function register_menu(): void {
		add_menu_page(
			__( 'Boiler Settings', \ASC_AI_BOILER_TEXT_DOMAIN ),
			__( 'Boiler Settings', \ASC_AI_BOILER_TEXT_DOMAIN ),
			'manage_options',
			self::PAGE_SLUG,
	...
	public function handle_save(): void {
		...
		if ( isset( $_POST['example_settings_task'] ) ) {
			$task = sanitize_key( (string) wp_unslash( $_POST['example_settings_task'] ) );
		}
	...
	public function render_settings_page(): void {
		...
		<div class="wrap example-settings-page example-admin-settings-hub">
			<h1><?php esc_html_e( 'Boiler Settings', \ASC_AI_BOILER_TEXT_DOMAIN ); ?></h1>
			...
			<h2><?php esc_html_e( 'Content', \ASC_AI_BOILER_TEXT_DOMAIN ); ?></h2>
			...
			<p class="example-admin-settings-hub__sync-intro">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . BoilerSettingsPage::PAGE_SLUG ) ); ?>"><?php esc_html_e( 'Backup / Restore', \ASC_AI_BOILER_TEXT_DOMAIN ); ?></a>
				<span class="description"><?php esc_html_e( ' Copy plugin HTML files to or from WordPress in batches.', \ASC_AI_BOILER_TEXT_DOMAIN ); ?></span>
			</p>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="example-settings-page__default-form">
				...
				<input type="hidden" name="example_settings_task" value="<?php echo esc_attr( self::TASK_SAVE_SETTINGS ); ?>">

				<h2><?php esc_html_e( 'Default Images', \ASC_AI_BOILER_TEXT_DOMAIN ); ?></h2>
				<p>
					<?php esc_html_e( 'Images should have a 4:3 aspect ratio. Recommended image size: 1440x1080. Setting the alt text description is advised for selected images.', \ASC_AI_BOILER_TEXT_DOMAIN ); ?>
				</p>
						...
						<?php foreach ( $image_fields as $field ) : ?>
							...
							<tr>
								...
								<td>
									...
									>
										<?php esc_html_e( 'Select Image', \ASC_AI_BOILER_TEXT_DOMAIN ); ?>
									</button>
									<button
										type="button"
										...
										data-target-input="<?php echo esc_attr( $key ); ?>"
									>
										<?php esc_html_e( 'Clear', \ASC_AI_BOILER_TEXT_DOMAIN ); ?>
									</button>
									<p class="description">
										<?php esc_html_e( 'Attachment ID:', \ASC_AI_BOILER_TEXT_DOMAIN ); ?>
				...
				<?php submit_button( __( 'Save default images', \ASC_AI_BOILER_TEXT_DOMAIN ) ); ?>
			</form>
		</div>
		<?php
	}
	...
	private function get_content_admin_links(): array {
		...
		$out[] = array(
			...
			'label' => __( 'Pages', \ASC_AI_BOILER_TEXT_DOMAIN ),
		);

		$out[] = array(
			'url' => admin_url( 'edit.php' ),
			'label' => __( 'Posts', \ASC_AI_BOILER_TEXT_DOMAIN ),
		);
	...
	private function get_image_fields(): array {
		return array(
			array(
				...
				'label' => __( 'Blog default image', \ASC_AI_BOILER_TEXT_DOMAIN ),
			),
			array(
				'key' => CoreSettings::SETTING_IMAGE_SERVICES,
				'label' => __( 'Services default image', \ASC_AI_BOILER_TEXT_DOMAIN ),
			),
			array(
				'key' => CoreSettings::SETTING_IMAGE_PROJECTS,
				'label' => __( 'Projects default image', \ASC_AI_BOILER_TEXT_DOMAIN ),
			),
		);
	}
	...
	public function handle_ajax_set_default_image(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'You do not have permission to update settings.', \ASC_AI_BOILER_TEXT_DOMAIN ),
				),
				403
			);
		}
		...
		if ( ! in_array( $setting_key, $allowed_keys, true ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Invalid settings key.', \ASC_AI_BOILER_TEXT_DOMAIN ),
				),
				400
			);
		}
